	...
	protected function compileCode( $parsed_code ){
        ...
        $loop_level = 0;
	 	...
	 	while( $html = array_shift( $parsed_code ) ){
			...
			//GET tag
			elseif( preg_match( '/\{get="([^"]*)"\}/', $html, $code ) ){

				//variables substitution
				$get_var = $this->var_replace( $code[ 1 ], $left_delimiter = null, $right_delimiter = null, $php_left_delimiter = '".' , $php_right_delimiter = '."', $loop_level );

        //dynamic include
        if(filter_var($get_var,FILTER_VALIDATE_URL,FILTER_FLAG_PATH_REQUIRED))
          $url = urlencode($get_var);
        else
          $url = $_SERVER['DOCUMENT_ROOT'].$get_var;
        
        $compiled_code .=  file_get_contents( $url );
			}
